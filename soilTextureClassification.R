...
#' @param data a data frame containing the percentage of sand, silt, and clay
#' @return textural class for each soil samples
#' @export textureClassif
#' @author Christian L. Goueguel,
#' christian.goueguel@gmail.com
#'
textureClassif <- function(data) {
  ...
  loamySand_class <- data %>%
    filter(
      data$silt + 1.5 * data$clay >= 15 &
      data$silt + 2 * data$clay < 30
      )
  sandyLoam_class <- data %>%
    filter(
      data$clay >= 7 &
      data$clay < 20 &
      data$sand > 52 &
  ...
  siltyClayLoam_class <- data %>%
    filter(
      data$clay >= 27 &
      data$clay < 40 &
      data$sand <= 20
      )
  sandyClay_class <- data %>%
    filter(
      data$clay >= 35 &
      data$sand > 45
      )
  siltyClay_class <- data %>%
    filter(
      data$clay >= 40 &
      data$silt >= 40
      )
  ...
  .textClass <- rbind(Fine_group, Medium_group, Coarse_group)
